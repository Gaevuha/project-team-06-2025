import{a as l,S as D}from"./assets/vendor-Breb73Hu.js";(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const o of a.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function e(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function n(s){if(s.ep)return;s.ep=!0;const a=e(s);fetch(s.href,a)}})();const v={artistsList:document.querySelector(".artists"),loadMoreBtn:document.querySelector(".load-more-btn"),learnMoreArtistBtns:document.querySelectorAll(".learn-more-artist-btn"),artists:document.querySelector(".list-artists"),modalAlbumList:document.querySelector(".modal-list__album")},I="/project-team-06-2025/assets/sprite-TQuCmG2s.svg",H="icon-youtube",U="learn-more";function j(t,r){if(!t)return"No description available";const e=t.indexOf(".");return e!==-1&&e<=r?t.slice(0,e+1).trim():t.length<=r?t.trim():t.slice(0,r).trim()+"..."}function Y(){const t=window.innerWidth;return t<768?60:t<1024?160:147}function S(){const t=Y(),r=h.map(e=>{const n=(e.genres||[]).map(o=>`<li class="artist-genre">${o}</li>`).join(""),s=e.strArtistThumb||"/img/placeholder-image-mini.jpg",a=j(e.strBiographyEN,t);return`
      ...
      `}).join("");v.artistsList.innerHTML=r}function T(){const t=document.querySelector(".loader");t.style.display="block"}function m(){const t=document.querySelector(".loader");t.style.display="none"}function N(){const t=document.querySelector(".load-more-btn");t.style.display="block"}function k(){const t=document.querySelector(".load-more-btn");t.style.display="none"}async function Q(t){const{_id:r,strArtist:e,strArtistThumb:n,intFormedYear:s,intMembers:a,strCountry:o,strGender:i,strBiographyEN:A,genres:d=[],albumsList:u=[]}=t,M=d.map(f=>`<li class="artist__genre-item">${f}</li>`).join("");console.log("genres:",d);const g=u.map(({strAlbum:f,intYearReleased:E,tracks:b=[]})=>{const q=b.map(({strTrack:C,intDuration:B,movie:c})=>{const F=Math.floor(B/6e4),O=String(Math.floor(B%6e4/1e3)).padStart(2,"0"),P=c!=null&&c.startsWith("http")?c:c?`https://${c}`:"";return`
        <li class="track__row">
          <span class="track__title">${C}</span>
          <span class="track__time">${F}:${O}</span>
          <span class="track__link">
            ${c?`<a href="${P}" target="_blank" aria-label="YouTube link">
